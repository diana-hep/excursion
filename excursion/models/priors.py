...
class LinealMean(gpytorch.means.Mean):
    def __init__(self, prior=None, batch_shape=torch.Size(), **kwargs):
        ...
        self.register_parameter(name="slope", parameter=torch.nn.Parameter(torch.zeros(*batch_shape, self.ndim, dtype=torch.float64)))
        self.register_parameter(name="constant", parameter=torch.nn.Parameter(torch.zeros(*batch_shape, 1, dtype=torch.float64)))
