...
def PES(gp, testcase, thresholds, device, dtype):
    """
    ...
    for x_candidate in X_grid:
        x_candidate = x_candidate.view(1, -1).to(device=device, dtype=dtype)
    ...
    acquisition_values_grid = torch.tensor(acquisition_values_grid, dtype=dtype, device=device)
